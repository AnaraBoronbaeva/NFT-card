window.addEventListener("DOMContentLoaded", function () {
  ...
  document.body.style.cssText = `background-color :hsl(217, 54%, 11%);
  width:1440px;
  margin:0 auto;
  ...
  cardTitle.style.cssText = `color:hsl(0, 0%, 100%);
  font-size:18px;
 `;

  card.append(cardDescrp);
  cardDescrp.textContent =
    "Our equilibruim collection promotes balance and calm";
  cardDescrp.style.cssText = `color:hsl(215, 51%, 70%);
 font-weight:300;
 font-size:15px;
 `;
...
`;
  ...
  ethereum.setAttribute("src", "/images/icon-ethereum.svg");

  info.append(ethereumDescrp);
  ethereumDescrp.textContent = "0.041 ETH";
  ethereumDescrp.style.cssText = `padding-left:10px;
...
`;
  days.append(daysImg);
  daysImg.setAttribute("src", "/images/icon-clock.svg");
...
border: none;`;
  ...
  userImg.setAttribute("src", "/images/image-avatar.png");
  user.style.cssText = `
margin-top:20px;
display:flex;
align-items:center;
